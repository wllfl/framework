    ...
    public function insert($arrayDados, $duplicidade=TRUE){
    	try{
	    	...
	    	if ($this->validaArray($arrayDados)):
	    		...
	    		if($retorno == 1):
	    			return Helper::printMsgConfim('Registro incluído com sucesso!');
	    		else:
	    			return Helper::printMsgErro('Houve um erro ao executar operação!');
	    		endif;
	    	else:
	    		return Helper::printMsgAviso('Existem campos sem preenchimento!');
	    		exit();
	    	endif;	
	    }catch(Exception $e){
    ...
    public function update($arrayDados, $arrayCondicao){
    	try{
	    	if ($this->validaArray($arrayDados)):
	    		...
	    		if($retorno == 1):
	    			return Helper::printMsgConfim('Registro alterado com sucesso!');
	    		else:
	    			return Helper::printMsgErro('Houve um erro ao executar operação!');
	    		endif;
	    	else:
    ...
    public function delete($arrayCondicao){
    	try{
	    	if ($this->validaArray($arrayCondicao)):
	    		...
	    		if($retorno == 1):
	    			return Helper::printMsgConfim('Registro excluído com sucesso!');
	    		else:
	    			return Helper::printMsgErro('Houve um erro ao executar operação!');
	    		endif;
	    	else:
